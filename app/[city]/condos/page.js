...
export default async function CondosPage({ params }) {
  ...
  const buildProductSchema = (property) => {
    ...
    return {
      "@type": "Product",
      "@id": url,
      name: property.project_name || `Pre Construction Condo in ${cityName}`,
      description: `New pre-construction Condo located at ${property.project_address || cityName}.`,
      image: [image],
  ...
  const itemListSchema =
    schemaProducts.length > 0
      ? {
          ...
          "@type": "ItemList",
          url: `https://condomonk.ca/${city}/condos`,
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Condos", item: `https://condomonk.ca/${city}/condos` },
    ],
  };
  // ────────────────────────────────────────────────────────────────────────
  ...
  const generateSubtitle = () => {
    ...
    if (city == "calgary" || city == "edmonton") {
      return (
        <>
          Explore 100+ new construction condos in {cityName}, AB from trusted builders. Updated floor plans, pricing & availability for 1–4+ bedroom units.{" "}
    ...
    return (
      <>
        Explore {data.preconstructions.length}+ pre construction condos in {cityName}, ON from trusted{" "}
        <Link href="/builders" className="text-slate-700 underline hover:text-slate-900">
          builders in {cityName}
        </Link>
        . Updated floor plans, pricing & availability for 1–4+ bedroom units.{" "}
        <span className="text-gray-500 text-sm">Last Updated: {lastUpdated}</span>
      </>
    );
  };
