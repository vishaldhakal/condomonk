...
export async function getProperties({
  ...
  try {
    ...
    // Add optimized image URLs directly without API calls
    const propertiesWithImages = data.value.map((property) => {
      return {
        ...property,
        imageUrl: {
          medium: `https://pillar9.homebaba.ca/images/${property.ListingKey}-0.jpg?maxSize=80`,
          thumbnail: `https://pillar9.homebaba.ca/images/${property.ListingKey}-0.jpg?cardImage=true&maxSize=50`,
        },
      };
    });
...
export const getImageUrls = async ({
  ...
  const countRes = await fetch(
    `https://query.ampre.ca/odata/Media?$select = MediaKey&$filter=ResourceRecordKey eq '${listingKey}' and ImageSizeDescription eq 'Large' and MediaStatus eq 'Active'`,
    options
  );
  ...
  try {
    ...
    const propertiesWithImages = data.value.map((property) => {
      return {
        ...property,
        images: [
          `https://pillar9.homebaba.ca/images/${property.ListingKey}-0.jpg`,
        ],
        imageUrl: {
          medium: `https://pillar9.homebaba.ca/images/${property.ListingKey}-0.jpg?maxSize=80`,
          thumbnail: `https://pillar9.homebaba.ca/images/${property.ListingKey}-0.jpg?cardImage=true&maxSize=50`,
...
export async function getBarrieCondos(limit = 8) {
  ...
  try {
    ...
    // Process properties to include optimized image URLs
