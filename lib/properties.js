export async function getProperties({
  ...
  filterClauses.push("StandardStatus eq 'Active'");

  // MLS Status for price drops
  if (mlsStatus === "Price Change") {
    filterClauses.push("MlsStatus eq 'Price Change'");
  }
...
export async function getRoomInformation(listingKey) {
  ...
  try {
    // Modified query to match the API structure
    const url = `${baseUrl}PropertyRoom?$filter=ResourceRecordKey eq '${listingKey}'`;

    console.log("Fetching room info from:", url); // Debug log
    ...
    if (!response.ok) {
      console.error("Room info response status:", response.status);
      console.error("Room info response statusText:", response.statusText);
      throw new Error(`HTTP error! status: ${response.status}`);
    }

    const data = await response.json();

    // Filter rooms after fetching to include only relevant room types
    const relevantRooms = data.value.filter((room) =>
      ["Bedroom", "Kitchen", "Dining", "Living", "Games"].some((type) =>
        room.RoomType?.includes(type)
      )
    );
