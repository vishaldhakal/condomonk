...
export async function getListingDetail(listingID) {
  ...
  try {
    ...
    const imagesUrl = `${baseUrl}Media?$filter=ResourceRecordKey eq '${listingKey}' and MediaStatus eq 'Active' and ImageSizeDescription eq 'Medium' and MediaType eq 'image/jpeg'&$top=70&$select=MediaURL,ShortDescription,PreferredPhotoYN,Order,MediaType,ImageSizeDescription`;
...
export async function getPropertiesAnalytics({
  ...
  const pageSize = 1000; // Larger page size since we need aggregate data
  const page = Number(searchParams.page) || 1;
  const skip = (page - 1) * pageSize;
  ...
  const queryString = [
    `$filter=${filterClauses.join(" and ")}`,
    `$select=ListingKey,ListPrice,PropertySubType,BedroomsTotal,BathroomsTotalInteger`, // Minimal fields
    `$count=true`,
  ].join("&");
