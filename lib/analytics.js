  ...
  try {
    ...
    const stats = {
      overview: {
        ...
        avgDaysOnMarket:
          calculateAverage(properties, "OriginalEntryTimestamp") || 0,
...
function calculateAverage(properties, field) {
  ...
  if (field === "OriginalEntryTimestamp") {
    const sum = properties.reduce(
      (acc, prop) =>
        acc +
        (Date.now() - new Date(prop[field]).getTime()) / (1000 * 60 * 60 * 24),
      0
    );
...
function calculateBedroomStats(properties, bedCount, isPlus = false) {
  ...
  return {
    ...
    avgDaysOnMarket: calculateAverage(filtered, "OriginalEntryTimestamp") || 0,
  };
}
